...
export default function OtherUser() {
  ...
  const { havMatPicArr, setHavMatPicArr, setGiveMatPicArr } = useContext(
    OtherUserContext
  );
  ...
  const getOtherUserInfo = async () => {
    ...
    if (token !== null) {
      ...
      setGiveMatPicArr(
        res.data.savedPics
          .filter((p) => p.matching.includes(userInfo._id))
          .map((obj) => obj._id)
      );
    }
  };
            ...
            <Box display="flex" flexDirection="column" alignItems="center">
              ...
              <Box width="300px" mt={2}>
                <Typography color="primary" variant="body2">
                  If you think the original and matching pic are taken at the
                  same place in the naturespot, give your vote to it
